...
{
    *mask = portENTER_CRITICAL_NESTED();
}

void BoardCriticalSectionEnd(uint32_t *mask)
{
    portEXIT_CRITICAL_NESTED(*mask);
}
...
void BoardInitMcu(void)
{
    ...
    UartConfig(
        &Uart1, RX_TX, 921600, UART_8_BIT, UART_1_STOP_BIT, NO_PARITY, NO_FLOW_CTRL);
...
{
    uint64_t chip_id = esp_efuse_mac_get_default();
    memcpy(id, &chip_id, 8);
}
